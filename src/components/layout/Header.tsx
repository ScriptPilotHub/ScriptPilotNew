        ...
        <div className={`flex items-center justify-between transition-all duration-500 ${
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navLinks.map(link => (
              ...
              >
                {link.label}
                <span className={`absolute bottom-[-4px] left-1/2 transform -translate-x-1/2 h-0.5 bg-teal-400 transition-all duration-300 ${
                  pathname === link.href.substring(1) || (pathname === '' && link.href === '#home')
                    ? 'w-full' 
          ...
          <div className="md:hidden">
            <button 
              ...
              className="text-slate-300 hover:text-white focus:outline-none transform hover:scale-110 transition-transform duration-200"
            >
            ...
            {navLinks.map(link => (
              <a
                ...
                className={`block px-4 py-3 rounded-lg text-base font-medium transition-all duration-300 ${
                  pathname === link.href.substring(1) || (pathname === '' && link.href === '#home')
                    ? 'bg-slate-800 text-teal-300 shadow-lg' 
